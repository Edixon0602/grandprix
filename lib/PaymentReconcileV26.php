...
final class PaymentReconcileV26
{
    ...
    public static function getAccount(string $customerKey): array
    {
        ...
        $weekly = self::numericValue($contract['row'], $contract['map']['weekly'] ?? null);
        if ($weekly <= 0) $weekly = self::numericValue($row, $map['weekly'] ?? null);
        ...
        $weeks = self::attachV26Balances($customerKey, $weeksContext['weeks']);
        ...
        foreach ($weeks as $week) {
            ...
            $summary['partial_paid_usd'] += (float)$week['v26_paid_usd'];
        }
    ...
    public static function preview(array $input): array
    {
        ...
        foreach ($account['weeks'] as $week) {
            ...
            $allocations[] = [
                ...
                'week_amount_usd' => round((float)$week['amount_usd'], 2),
        ...
        return [
            ...
            'applied_usd' => round($money['amount_usd'] - max(0, $remaining), 2),
    ...
    private static function discoverCustomerSources(): array
    {
        ...
        foreach (self::tables() as $table) {
            if (str_starts_with($table, 'gp_v26_')) continue;
    ...
    private static function customerSummary(array $source, array $row): array
    {
        ...
        return [
            ...
            'weekly_amount_usd' => self::numericValue($row, $map['weekly'] ?? null),
    ...
    private static function resolveContractContext(array $source, array $customerRow): array
    {
        ...
        $best = null;
        foreach (self::tables() as $table) {
            if ($table === $source['table'] || str_starts_with($table, 'gp_v26_')) continue;
            $cols = self::columns($table);
    ...
    private static function attachV26Balances(string $customerKey, array $weeks): array
    {
