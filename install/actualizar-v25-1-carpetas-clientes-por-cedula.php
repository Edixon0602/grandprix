...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(!gp_verify_csrf(is_string($csrf)?$csrf:null)){$error='Sesión de seguridad vencida.';}
        ...
        try{
            ...
            if(!is_dir($root)&&!mkdir($root,0750,true)&&!is_dir($root))throw new RuntimeException('No se pudo crear config/clientes. Verifica permisos de escritura.');
            $ht=$root.'/.htaccess';
            ...
            $pendingCedulas=(int)$pdo->query("SELECT COUNT(*) FROM gp_finance_accounts WHERE record_status <> 'archived' AND (identity_document IS NULL OR TRIM(identity_document)='')")->fetchColumn();
            $summary=[
                ...
                'Clientes con cédula pendiente'=>$pendingCedulas,
