...
if(!gp_user_can('finance.clients.edit')&&!gp_user_can('finance.payments.reconcile')&&!gp_user_can('users.permissions')){
    http_response_code(403);exit('No tienes permiso para instalar la actualización unificada V31.');
}
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    if(!gp_verify_csrf((string)($_POST['csrf']??'')))$error='Sesión de seguridad vencida. Recarga la página.';
        ...
        foreach($payments as $pay){
            ...
            $applied=round($amountUsd-max(0,$remaining),2);$pdo->prepare('UPDATE gp_finance_payments SET installments_applied=?,week_numbers_json=?,amount_usd=?,applied_usd=?,unapplied_usd=? WHERE id=?')->execute([count(array_unique($completed)),json_encode(array_values(array_unique($touched))),$amountUsd,$applied,max(0,$remaining),$pid]);
        ...
        // V31: normaliza todos los movimientos ya distribuidos. installments_applied = cuotas 100% cerradas;
        // week_numbers_json = todas las semanas tocadas (incluye la amarilla con abono).
        $distributionFixed=0;
        $q=$pdo->query("SELECT p.id,p.account_id FROM gp_finance_payments p WHERE p.status='confirmed' AND EXISTS(SELECT 1 FROM gp_finance_payment_allocations a WHERE a.payment_id=p.id) ORDER BY p.id");
        foreach($q->fetchAll() as $pay){
            $pid=(int)$pay['id'];$aid=(int)$pay['account_id'];
            $a=$pdo->prepare('SELECT installment_no,completed FROM gp_finance_payment_allocations WHERE payment_id=? ORDER BY id');$a->execute([$pid]);$rows=$a->fetchAll();
            $touched=[];$completed=[];foreach($rows as $r){$w=(int)$r['installment_no'];$touched[]=$w;if((int)$r['completed']===1)$completed[]=$w;}
            $touched=array_values(array_unique($touched));$completed=array_values(array_unique($completed));
            $pdo->prepare('UPDATE gp_finance_payments SET installments_applied=?,week_numbers_json=? WHERE id=?')->execute([count($completed),json_encode($touched),$pid]);
            v28_refresh_receipt($pdo,$pid,$aid,$service);$distributionFixed++;
        }

        $service->refreshAll();
        $summary=['Clientes con cuota sincronizada'=>$accountsSynced,'Cuotas importadas desde V27'=>$plansImported,'Pagos anteriores reparados'=>$repaired,'Movimientos V31 normalizados'=>$distributionFixed,'Pagos automáticos USD/BS'=>'Activos','Semanas verdes + abono amarillo'=>'Activo','Recibos/PDF con distribución'=>'Activo','Reportes con pagadas + abono'=>'Activo','Resumen Ejecutivo interactivo'=>'Activo','Responsive Administración'=>'Activo','Responsive Mi GRANDPRIX'=>'Activo','Analítica por día / semana / mes'=>'Activa','10% USD y Bs. por período'=>'Activo','Error v17Date'=>'Corregido'];
        if($repairDetails)$warnings=array_slice($repairDetails,0,20);
        $done=true;
    }catch(Throwable $e){$error=$e->getMessage();}
}
$csrf=gp_csrf_token();
?><!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>GRANDPRIX V31 · Actualización unificada</title><style>*{box-sizing:border-box}body{margin:0;background:#eef4f9;font-family:Inter,Arial,sans-serif;color:#082744}.wrap{max-width:900px;margin:40px auto;padding:18px}.card{background:#fff;border:1px solid #dce7f0;border-radius:26px;padding:30px;box-shadow:0 25px 70px rgba(8,39,68,.1)}.tag{display:inline-block;padding:7px 11px;border-radius:999px;background:#eaf4ff;color:#1268ce;font-weight:900;font-size:11px}h1{margin:12px 0 8px}.lead{color:#617c93;line-height:1.6}.ok,.err,.warn{padding:16px;border-radius:16px;margin:18px 0}.ok{background:#eaf9f2;color:#116f4e}.err{background:#fff0f2;color:#a33d50}.warn{background:#fff8e9;color:#8a630d}.grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.grid div{padding:12px;border:1px solid #e5edf4;border-radius:14px}.grid small,.grid b{display:block}.grid small{color:#7890a4;font-size:9px;text-transform:uppercase}.grid b{margin-top:4px}button{border:0;border-radius:13px;padding:14px 20px;background:#1268ce;color:#fff;font-weight:900;cursor:pointer}a{color:#1268ce;font-weight:800}@media(max-width:620px){.wrap{margin:10px auto;padding:10px}.card{padding:20px 15px}.grid{grid-template-columns:1fr}}</style></head><body><div class="wrap"><div class="card"><span class="tag">GRANDPRIX V31 · ACTUALIZACIÓN UNIFICADA</span><h1>Pagos automáticos + Resumen Ejecutivo + móvil + analítica de cobranza</h1><p class="lead">Esta instalación es acumulativa: incorpora pagos por monto, multimoneda y abonos parciales; tarjetas interactivas, responsive móvil y la analítica de cobranza y la corrección V31 que sincroniza semanas completadas, abonos parciales, recibos, reportes y vista previa de conciliación.</p>
<?php if($done): ?><div class="ok"><b>✓ Actualización unificada V31 completada.</b><div class="grid"><?php foreach($summary as $k=>$v): ?><div><small><?=htmlspecialchars((string)$k)?></small><b><?=htmlspecialchars((string)$v)?></b></div><?php endforeach; ?></div></div><?php if($warnings): ?><div class="warn"><b>Pagos recalculados durante la instalación</b><br><?php foreach($warnings as $w): ?>• <?=htmlspecialchars($w)?><br><?php endforeach; ?></div><?php endif; ?><p><a href="../index.php">Volver a GRANDPRIX</a></p>
<?php elseif($error): ?><div class="err"><b>No se pudo completar la actualización.</b><br><?=htmlspecialchars($error)?></div><?php endif; ?>
<?php if(!$done): ?><form method="post"><input type="hidden" name="csrf" value="<?=htmlspecialchars($csrf)?>"><button type="submit">Instalar V31 completa</button></form><?php endif; ?></div></div></body></html>
